...
func pixivSearch(ctx context.Context, query string) ([]pixivItem, error) {
	...
	var raw struct {
		...
		Body    struct {
			IllustManga struct {
				Data []pixivItem `json:"data"`
			} `json:"illustManga"`
		} `json:"body"`
	}
	if err := pixivGet(ctx, endpoint, &raw); err != nil {
		return nil, err
	}
	if raw.Error {
		return nil, fmt.Errorf("%s", raw.Message)
	}
...
func pixivDetailGet(ctx context.Context, id string) (pixivDetail, error) {
	...
	if err := pixivGet(ctx, pixivBase+"/ajax/illust/"+url.PathEscape(id), &raw); err != nil {
		return pixivDetail{}, err
	}
	if raw.Error || raw.Body.ID == "" {
		return pixivDetail{}, fmt.Errorf("artwork tidak ditemukan")
	}
	tags := make([]string, 0, len(raw.Body.Tags.Tags))
	for _, tag := range raw.Body.Tags.Tags {
...
func pixivHandler(ctx context.Context, c *command.Ctx) error {
	...
	if _, err := strconv.ParseInt(query, 10, 64); err != nil {
		...
		if err != nil {
			return pixivError(ctx, c, "Gagal mencari di Pixiv: "+err.Error())
		}
		if len(results) == 0 {
			return pixivError(ctx, c, "Tidak menemukan artwork untuk kata kunci itu.")
		}
		limit := len(results)
		if limit > 15 {
			limit = 15
		...
		id = results[rand.Intn(limit)].ID
	}
	detail, err := pixivDetailGet(ctx, id)
	if err != nil {
		return pixivError(ctx, c, "Gagal mengambil detail artwork: "+err.Error())
	}
	...
	for _, tag := range detail.Tags[:minInt(len(detail.Tags), 7)] {
	...
	if detail.PageCount > 1 {
		caption += fmt.Sprintf("📄 *Pages:* %d\n", detail.PageCount)
	}
	if detail.BookmarkCount > 0 {
		caption += fmt.Sprintf("❤️ *Bookmarks:* %d\n", detail.BookmarkCount)
	}
	if len(tags) > 0 {
		caption += "🏷️ *Tags:* " + strings.Join(tags, " ") + "\n"
	}
...
func pixivError(ctx context.Context, c *command.Ctx, message string) error {
