// Package exif menangani parsing dan injeksi metadata EXIF WhatsApp pada WebP sticker.
package exif
...
const defaultStickerPackID = "com.rbot.sticker"

// AddStickerExif menambahkan metadata EXIF WhatsApp (packname & author) ke WebP.
func AddStickerExif(webp []byte, packname, author string) ([]byte, error) {
	...
	exifPayload, err := stickerExifPayload(packname, author)
	if err != nil {
		return nil, err
	}
	...
	chunks[insertAt] = webpChunk{kind: "EXIF", payload: exifPayload}

	var out bytes.Buffer
	out.Grow(len(webp) + 8 + len(exifPayload) + 1)
...
func stickerExifPayload(packname, author string) ([]byte, error) {
	metadata, err := json.Marshal(map[string]any{
		"sticker-pack-id":        defaultStickerPackID,
