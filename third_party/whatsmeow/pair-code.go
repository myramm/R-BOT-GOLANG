...
func (cli *Client) handleCodePairNotification(ctx context.Context, parentNode *waBinary.Node) error {
	...
	if !ok {
		// WhatsApp kini (flow passkey Meta, sejak akhir Juni 2026) dapat mengirim
		// notifikasi link_code_companion_reg tanpa wrapped primary ephemeral pub
		// setelah kode diterima & passkey diverifikasi. Notifikasi ini adalah
		// prekursor handoff passkey: jangan coba path legacy companion_finish,
		// biarkan handler passkey_prologue_request melanjutkan pairing
		// (lihat whatsmeow issue #1233 / #1188 dan PR #1234).
		return nil
	}
